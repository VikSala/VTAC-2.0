...
def update_custom_stock():
    ...
    # --- helpers -------------------------------------------------------------
    def fields_meta(model):
        return models.execute_kw(
            db, uid, password, model, "fields_get", [],
            {"attributes": ["type", "relation"]}
        )

    def build_zero_vals(model_fields):
        """Devuelve vals con el cero correcto según tipo y solo para campos existentes."""
        vals = {}
        for f in CUSTOM_FIELDS:
            meta = model_fields.get(f)
            if not meta:
                continue
            ...
            if ftype in ("float", "monetary"):
                ...
                vals[f] = 0
            elif ftype in ("boolean", "many2one"):
                vals[f] = False
            else:
                # Para char/text/selection no tocamos (evita sorpresas)
                continue
        return vals

    # --- 1) Buscar plantillas por marca -------------------------------------
    tmpl_ids = models.execute_kw(
        db, uid, password,
        "product.template", "search",
        [[("product_brand_id.name", "=", BRAND_NAME)]]
    )
    if not tmpl_ids:
        print(f"⚠ No se encontraron productos con marca '{BRAND_NAME}'.")
        return {"templates": 0, "variants": 0}

    # --- 2) Preparar valores a cero según tipos ------------------------------
    tmpl_fields = fields_meta("product.template")
    prod_fields = fields_meta("product.product")
    vals_tmpl = build_zero_vals(tmpl_fields)
    vals_prod = build_zero_vals(prod_fields)

    # Nada que actualizar
    if not vals_tmpl and not vals_prod:
        print("ℹ Ninguno de los campos custom existe en product.template ni product.product.")
        return {"templates": 0, "variants": 0}

    # --- 3) Actualizar en product.template ----------------------------------
    updated_templates = 0
    if vals_tmpl:
        models.execute_kw(
            db, uid, password,
        ...
        updated_templates = len(tmpl_ids)

    # --- 4) Actualizar en product.product (variantes) ------------------------
    updated_variants = 0
    if vals_prod:
        ...
        if prod_ids:
            ...
            updated_variants = len(prod_ids)

    print(
        f"✅ Marca '{BRAND_NAME}': {updated_templates} plantillas y {updated_variants} variantes actualizadas a cero.")
    return {"templates": updated_templates, "variants": updated_variants}

def update_comercial_stock(excel_path):
    from App_Connection import db, uid, password, models

    print("Actualizando stock campos custom...")
    ...
    # --- Helpers -------------------------------------------------------------
    def _none_to_false(vals: dict) -> dict:
    ...
    def _is_empty_cell(v):
        try:
            if pd.isna(v):  # NaN/None/NaT
                return True
        except Exception:
            pass
        return (v is None) or (isinstance(v, str) and v.strip() == "") or (str(v).strip().lower() == "nan")
    ...
    # ¿Dónde existen los campos?
    prod_fields = fields_meta("product.product")
    tmpl_fields = fields_meta("product.template")
    has_madrid_on_product = FIELD_MADRID in prod_fields
    has_madrid_on_template = FIELD_MADRID in tmpl_fields
    has_bulgaria_on_product = FIELD_BULGARIA in prod_fields
    has_bulgaria_on_template = FIELD_BULGARIA in tmpl_fields
    if not (
            has_madrid_on_product or has_madrid_on_template or has_bulgaria_on_product or has_bulgaria_on_template):
        print(f"⚠ Ninguno de los campos existe: '{FIELD_MADRID}', '{FIELD_BULGARIA}'.")
        return {"updated_madrid": 0, "updated_bulgaria": 0, "attr_set": 0}
    ...
    df_madrid = _normalize_columns(xls[SHEET_MADRID])
    df_bulg = _normalize_columns(xls[SHEET_BULGARIA])

    # La hoja Odoo puede no existir; si no está, df_odoo=None
    try:
        df_odoo = _normalize_columns(xls[SHEET_ODOO])
    except KeyError:
        df_odoo = None
        print("ℹ Hoja 'Odoo' no encontrada; la condición adicional no podrá cumplirse.")

    # Columnas esperadas
    for need, df, sheet in [
        (("SKU", "STOCK"), df_madrid, SHEET_MADRID),
        (("SKU", "STOCK", "UNDELIVERED ORDER"), df_bulg, SHEET_BULGARIA),
    ]:
        missing = [c for c in need if c not in df.columns]
        if missing:
            raise RuntimeError(f"[{sheet}] faltan columnas en Excel: {missing}")

    # Limpiamos filas sin SKU
    df_madrid = df_madrid[df_madrid["SKU"].notna()]
    df_bulg = df_bulg[df_bulg["SKU"].notna()]

    # --- Construir set de SKUs con STOCK < 1 en hoja Odoo --------------------
    odoo_low_stock_skus = set()
    if df_odoo is not None:
        if "SKU" in df_odoo.columns and "STOCK" in df_odoo.columns:
            for _, r in df_odoo.iterrows():
                sku_o = _coerce_sku(r["SKU"])
                if not sku_o:
                    continue
                stk_o = _to_float(r["STOCK"], default=0.0)
                if stk_o < 1:
                    odoo_low_stock_skus.add(sku_o)
        else:
            print("ℹ Hoja 'Odoo' no tiene columnas 'SKU' y 'STOCK'; se omite condición adicional.")

    # --- Índice de productos por SKU (default_code) --------------------------
    def build_sku_index(skus):
        skus = sorted({_coerce_sku(s) for s in skus if _coerce_sku(s)})
        sku_map = {}  # sku -> {"product_id": id or None, "template_id": id or None}
        ...
        for i in range(0, len(skus), CHUNK):
            ...
            for r in rows:
                ...
                if code:
                    sku_map[code] = {"product_id": r["id"], "template_id": r["product_tmpl_id"][0]}

        # Plantillas para SKUs no encontrados como variantes
        ...
        for i in range(0, len(missing), CHUNK):
            ...
            code_to_tmpl = {(r.get("default_code") or "").strip(): r["id"] for r in rows}
            for code in sub:
                if code in code_to_tmpl:
                    sku_map[code] = {"product_id": None, "template_id": code_to_tmpl[code]}
                else:
                    sku_map[code] = {"product_id": None, "template_id": None}
        return sku_map

    all_skus = list(df_madrid["SKU"].astype(str)) + list(df_bulg["SKU"].astype(str))
    sku_index = build_sku_index(all_skus)

    # --- Utilidades de escritura --------------------------------------------
    def write_field_for_sku(sku, field_name, value):
    ...
    # ==========================
    # 1) MADRID → x_transit_stock_custom
    # ==========================
    updated_madrid = 0
    for _, row in df_madrid.iterrows():
        ...
        if not sku:
            continue
        stock_val = _to_float(row["STOCK"], default=0.0)  # tal cual

        if write_field_for_sku(sku, FIELD_MADRID, stock_val): updated_madrid += 1

    # ==========================
    # 2) BULGARIA → We Will Have AND not Stock
    # ==========================
    updated_bulgaria = 0
    proximamente_skus = []

    for _, row in df_bulg.iterrows():
        sku = _coerce_sku(row["SKU"])
        if not sku:
            continue

        # Normalizar stock: negativos → 0
        stock_val = _to_float(row["STOCK"])
        if stock_val < 0:
            stock_val = 0.0

        if write_field_for_sku(sku, FIELD_BULGARIA, stock_val): updated_bulgaria += 1

        # Condición 1: UNDELIVERED ORDER no vacío (no NaN/None/""/nan) y != 0
        undeliv = row.get("UNDELIVERED ORDER", None)
        has_undelivered = not _is_empty_cell(undeliv)

        # Condición 2: STOCK Bulgaria <= 0
        cond_stock_bulg = (stock_val <= 0)

        # Condición 3: En hoja Odoo, el SKU tiene STOCK < 1
        cond_odoo = (sku in odoo_low_stock_skus)
    ...
    try:
        unique_skus = sorted(set(proximamente_skus))
        df_prox = pd.DataFrame({"SKU": unique_skus})

        try:
            with pd.ExcelWriter(excel_path, engine="openpyxl", mode="a", if_sheet_exists="replace") as writer:
                df_prox.to_excel(writer, sheet_name=SHEET_PROX, index=False)
        except TypeError:
            # pandas antiguo (sin if_sheet_exists)
            from openpyxl import load_workbook
            book = load_workbook(excel_path)
            if SHEET_PROX in book.sheetnames:
                del book[SHEET_PROX]
        ...
        print(f"⚠ No se pudo escribir la hoja '{SHEET_PROX}': {e}")

    print(f"✅ Madrid: {updated_madrid} SKUs → {FIELD_MADRID}")
    print(f"✅ Bulgaria: {updated_bulgaria} SKUs → {FIELD_BULGARIA}")
    print("25%")

    return {
        "updated_madrid": updated_madrid,
        "updated_bulgaria": updated_bulgaria,
    }

def update_names(excel_path):
    nuevos_vz = []

    def set_prefix_vz_by_sku(sku, siglas):
        import re
        from App_Connection import db, uid, password, models

        sku = str(sku).strip().removesuffix(".0")
        ...
        v_ids = models.execute_kw(
            ...
            [[["default_code", "=", sku]]],
            {"fields": ["name", "product_tmpl_id"], "context": ctx, "limit": 1}
        )

        if not v_ids or "**" in (v_ids[0].get("name") or ""):
            return

        tmpl_id = v_ids[0]["product_tmpl_id"][0]
        ...
        old = t["name"]
        new = re.sub(r"^\[[A-Za-z]{2}([^\]]*)\]", rf"[{siglas}\1]", old)
        ...
        if siglas == "VZ": nuevos_vz.append(tmpl_id)

    def update_prefixes_from_excel():
        ...
        try:
            ...
            print(f"❌ Error leyendo el Excel '{excel_path}', hoja '{SHEET_NAME}': {e}")
            return

        # Asegurar columnas (si falta alguna, la creamos vacía para evitar KeyError)
        for col in COLUMNS:
            if col not in df.columns:
                df[col] = None

        # Mapa SKU -> columnas donde aparece (para detectar conflictos)
        sku_to_cols = {}
        for col in COLUMNS:
            for raw in df[col].tolist():
                ...
                if not sku:
                    continue
                sku_to_cols.setdefault(sku, set()).add(col)

        if not sku_to_cols:
            print("⚠ No se encontraron SKUs válidos en las columnas VS/VN/VD/VZ.")
            return

        # SKUs no ambiguos: exactamente una columna
        unambiguous = {sku: next(iter(cols)) for sku, cols in sku_to_cols.items() if len(cols) == 1}
        # Conflictos: más de una columna
        conflicts = {sku: sorted(cols) for sku, cols in sku_to_cols.items() if len(cols) > 1}

        if conflicts:
            print(f"⚠ Conflictos detectados (SKU en varias columnas): {len(conflicts)}")
            # Si quieres listarlos, descomenta:
            # for sku, cols in list(conflicts.items())[:30]:
            #     print(f"  {sku} -> {cols}")

        print(f"➡ SKUs a actualizar (no ambiguos): {len(unambiguous)}")

        updated_count = 0
        processed = 0

        for sku, col in unambiguous.items():
            set_prefix_vz_by_sku(sku, col)  # usa tu función base tal cual
            processed += 1
            # Nota: el conteo real de actualizados se imprime desde tu función base.

        print("----- RESUMEN update_prefixes_from_excel -----")
        print(f"Procesados (no ambiguos): {processed}")
        print(f"Conflictos (omitidos): {len(conflicts)}")
    update_prefixes_from_excel()
    print("50%")

    def update_vz_products(vz_tmpl_ids):
        from App_Connection import db, uid, password, models

        ctx = {"active_test": False, "lang": "es_ES", "website_id": False}
        ...
        print(f"➡ VZ nuevos a procesar: {len(vz_tmpl_ids)}")

        for tmpl_id in vz_tmpl_ids:
            try:
                # 1️⃣ Campos base
                models.execute_kw(
                ...
                # 2️⃣ Galería → crear DIRECTAMENTE
        ...
        print(f"✅ VZ procesados: {len(vz_tmpl_ids)}")
        print("75%")
...
def update_out_of_stock_msg_from_excel(excel_path):
    ...
    # --- Normalizar SKUs y stocks ---
    madrid = {normalize_sku(row["SKU"]): int(float(row["STOCK"])) if str(row["STOCK"]).strip() not in ("nan",
                                                                                                       "None") else 0
              for _, row in df_mad.iterrows() if normalize_sku(row["SKU"])}
    bulgaria = {normalize_sku(row["SKU"]): int(float(row["STOCK"])) if str(row["STOCK"]).strip() not in ("nan",
                                                                                                         "None") else 0
                for _, row in df_bul.iterrows() if normalize_sku(row["SKU"])}
    proximamente = {normalize_sku(row["SKU"]) for _, row in df_prox.iterrows() if normalize_sku(row["SKU"])}

    print(f"➡ SKUs Madrid: {len(madrid)}, Bulgaria: {len(bulgaria)}, Proximamente: {len(proximamente)}")

    updated = 0
    for sku in set(list(madrid.keys()) + list(bulgaria.keys()) + list(proximamente)):
        if not sku:
            continue

        msg, extra_fields = None, {}

        # --- Reglas ---
        if sku in madrid and madrid[sku] > 0:
            msg = MSG_MADRID
        elif sku in bulgaria and bulgaria[sku] > 0 and sku not in madrid:
            msg = MSG_BULGARIA
        elif sku in proximamente and sku not in madrid:
            msg = MSG_PROX
            extra_fields["allow_out_of_stock_order"] = False
        ...
        v_ids = models.execute_kw(
            ...
            "product.product", "search_read",
            [[["default_code", "=", sku]]],
            {"fields": ["name", "product_tmpl_id"], "context": ctx}
        )

        if not v_ids or "**" in v_ids[0].get("name"):
            return
